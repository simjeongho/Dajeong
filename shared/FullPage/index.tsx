...
const FullPage = ({ page, children, onNext, onPrev }: Props) => {
	...
	const handleScroll = useCallback(
		(e: WheelEvent) => {
			...
			const currentScrollTop = wrapperRef.current?.scrollTop ? Math.floor(wrapperRef.current?.scrollTop) : 0;

			if (currentScrollTop - 1 > heightList[page] || heightList[page] > currentScrollTop + 1) return;
			//아래로 이동
			if (e.deltaY > 0) {
				//스크룰 가장 아래까지 가지 않은 경우 리턴
				const currentHeight = Math.floor(nodes[page].scrollTop + nodes[page].offsetHeight);
				if (currentHeight - 1 > nodes[page].scrollHeight || nodes[page].scrollHeight > currentHeight + 1) return;
	...
	const initHeightList = (): number[] => {
		if (wrapperRef.current) {
			...
			const heights: number[] = Array.from(nodes).map((node: any, idx: number) => {
